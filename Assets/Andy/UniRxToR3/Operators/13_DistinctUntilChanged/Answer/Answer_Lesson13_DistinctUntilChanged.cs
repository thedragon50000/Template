    ...
    public class Answer_Lesson_13_DistinctUntilChanged : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            */
            ...
            // controller.ObserveEveryValueChanged(x => x.isGrounded)   //Note: 舊
            Observable.EveryValueChanged(controller,x => x.isGrounded)
                .Subscribe(isGrounded => StatusOutput(isGrounded));
            ...
            */
            ...
            Observable.EveryValueChanged(controller,x => x.isGrounded)
                .Subscribe(isGrounded => StatusOutput(isGrounded))
                .AddTo(gameObject); //this.gameObjectが破棄されたらDisposeされるようにする
